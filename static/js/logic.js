...
d3.json(link).then(function(data) {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        L.circleMarker([earthquake.geometry.coordinates[1], earthquake.geometry.coordinates[0]], {
            ...
            fillOpacity: 0.75
        }).bindPopup(`<h3> ${earthquake.properties.place}</h3><hr><strong>Magnitude:</strong> ${magnitude}<br><strong>Depth:</strong> ${depth}<br><strong>Latitude:</strong> ${earthquake.geometry.coordinates[1]} <br><strong>Longitude:</strong> ${earthquake.geometry.coordinates[0]}`).addTo(myMap);
    }
